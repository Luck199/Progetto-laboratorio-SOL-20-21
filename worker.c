...
void* vitaWorker(void*  idWorker)
{
	...
	int fdDaElaborare=0;
	while (getSegnale() !=2 || (getSegnale() != 1 && getNumClient()>0) )//stopWorker == 0)
	{
		accediCodaComandi();
...
//
//					accediStrutturaFile();
//					lockFileReturnValue=lockFileServer(path,fdDaElaborare);
//					lasciaStrutturaFile();
//
//					if(lockFileReturnValue != 1)
//					{
...
//
//					accediStrutturaFile();
//					unlockFileReturnValue=unlockFileServer(path,fdDaElaborare);
//					lasciaStrutturaFile();
//
//					if(unlockFileReturnValue != 1)
//					{
